# sst glyphs for aggregated data
...
# plot seasonalities no world map in background ########
sst_seasonal_models_world <- plyr::dlply(dplyr::select(sst_df,long,lat,sst,year,cyc_month)
                                         ,c("long", "lat"), function(df) {
                                           lm(sst ~ year + factor(cyc_month), data = df)
                                         })
sst_month_grid <- expand.grid(year=2017, cyc_month=1:12)

sst_month_preds_world <- ldply(sst_seasonal_models, function(mod) {
  sst_month_grid$pred <- predict(mod, newdata = sst_month_grid)
  sst_month_grid
})
sst_month_preds_world <- glyphs(sst_month_preds, "long", "cyc_month", "lat", "pred")

sst_seasonal_plot_world <- ggplot(sst_month_preds_world, aes(gx, gy, group=gid)) +
...
saveRDS(sst_seasonal_plot_world, "results/glyph/sst_seasonal_plot_no_countries.rds")

# plot seasonalities with world map in background ######
...
# plot deseasonalised data with world map and aggregated #####

sst_smoothed_models_world <- dlply(dplyr::select(sst_df,long,lat,sst,month,cyc_month), 
                                   c("long", "lat"), function(df) {
                                     gam(sst ~ s(month) + factor(cyc_month), data = df)
                                   })
saveRDS(sst_smoothed_models_world, "results/glyph/sst_smoothed_models_world.rds")
# sst_smoothed_models <- readRDS("results/glyph/sst_smoothed_models.rds")
sst_pred_grid <- expand.grid(month = seq(972, 1403, length=50),
                             cyc_month = 1)

sst_smoothed_preds_world <- ldply(sst_smoothed_models_world, function(mod) {
...
sst_smoothed_plot_world <- ggplot(sst_smoothed_preds_world, aes(gx, gy, group = gid)) + 
  add_ref_boxes(sst_smoothed_preds_world, color = "grey90") +
  add_ref_lines(sst_smoothed_preds_world, color = "grey90") +
  geom_path() + 
  theme_bw() +
  labs(x = "", y = "")
sst_smoothed_plot_world
saveRDS(sst_smoothed_plot, "results/glyph/sst_smoothed_plot_world.rds")

# plot deseasonlaised data locally scaled, aggregated #####

sst_smoothed_preds_world2 <- ddply(sst_smoothed_preds_world, c("long", "lat"), mutate,
...
sst_smoothed_scaled_colour_plot_world <- ggplot(sst_smoothed_preds_world2) +
  geom_tile(aes(long, lat, fill = range), data = grid, alpha = 0.5) + #alpha=0.5
  add_ref_boxes(sst_smoothed_preds_world2, color= "grey90") +
...
sst_smoothed_scaled_colour_plot_world
...
saveRDS(sst_seasonality_window_plot, "results/glyph/sst_seasonality_window_plot.rds")




# plot deseasonalised small window
sst_smoothed_models_world <- dlply(dplyr::select(sst_df,long,lat,sst,month,cyc_month), 
                                   c("long", "lat"), function(df) {
                                     gam(sst ~ s(month) + factor(cyc_month), data = df)
                                   })
...
# plot deseasonalised small window scaled locally
...
saveRDS(sst_smoothed_scaled_colour_plot, "results/glyph/sst_smoothed_scaled_colour_plot_window.rds")
