...
model_folder <- "cv-lasso-anam-diff-stand"
...
plot_predictions_best_l(err_mat = err_mat, model_list = model_list, ids = ids, 
                        ...
                        lambdas = lambdas, save_to = save_to, diff_features = TRUE)
...
# diff cv and eval data
ndiff <- 1
sst_anam_cv <- diff_data(sst_anam_cv, ndiff)
precip_cv <- precip_cv[-c(seq(ndiff))]

sst_anam_eval <- diff_data(sst_anam_eval, ndiff)
precip_eval <- precip_eval[-c(seq(ndiff))]
