...
lasso_des_corr_pre <- cv_for_ts(features_cv3, target_cv3, nfold = 5, size_train = 60, size_test = 14,
             
                                                 save_folder = "cv-lasso-on-corr-des")
# plot results
ids <- createTimeSlices(1:370, initialWindow=60, horizon=14,
                        skip=60+14-1)
lambdas <- readRDS("results/CV-lasso/cv-lasso-on-corr-des/lambda-vec.rds")
err_mat <- readRDS("results/CV-lasso/cv-lasso-on-corr-des/err-mat.rds")
plot_and_save_cv_results(err_mat, 5, ids, lambdas, features_cv3, target_cv3,
                         save_to = "results/CV-lasso/cv-lasso-on-corr-des/")
