...
file <- nc_open("../../data/interim/sst-interim.nc")
data <- ncvar_get(file, "sst")
lon <- ncvar_get(file, "lon")
lat <- ncvar_get(file, "lat")
fill_val <- ncatt_get(file, "sst", "_FillValue")
nc_close(file)
...
plot(r)
```
Next we inspect the development of the monthly mean temperatures over time
