...
library(ggplot2)
# load model
full_mod <- readRDS("results/CV-lasso/fused-full-20k//full-model.rds")
...
df <- data.frame(preds = preds$fit[,minmin], target = precip)
#dim(preds_full)
ggplot() + geom_line(data = df, mapping = aes(x=1:62, y=preds, colour = "blue")) +
  geom_line(data = df, mapping= aes(x=1:62, y=target)) + ggtitle("20k")


table(full_mod$beta[,minmin])
