...
early warning indicator for Amazon droughts or a drought index, based on tropical Atlantic sea surface 
...
Meaning, we seek to build a model that is able to predict droughts based on preceding
sea temperatures, desirably with as much lead time as possible. Also we want to 
identify those sea regions that are most important for doing so, making interpretability point of interest, too. A first model could be a LASSO approach
...
For this they use two datasets, one for the Sea Surface Temperatures (@smith2008improvements) and one for
the Rainfall (@funk2015climate) with monthly data for the time period of 1981 until 2016. The data can be downloaded for example in netcdf format and can be manipulated conventiently with Climate Data Operators (CDO, @schulzweida2019cdo). CDO in turn can be used with wrappers for R and Python. The data is organized on a longitude/latitude grid.  
They identify 4 oceanic regions that correlate the most with rainfall in the amazon basin, using a coupled network approach.
Figure \@ref(fig:cross) shows the cross degree towards rainfall in the Central Amazon Basin (CBA, blue box), for positive and negative correlations. Darker shades indicate a larger cross degree, hence a larger number of links and correlations with rainfall at more grid points in the CBA.  
The correlations are measured using a spearson rank-order correlation coefficient\
\[
...
\[
A_{ij} = \begin{cases}0p_{ij} < p_{th}\\ 1p_{ij}\geq p_{th}\\\end{cases}  .
\]
...
SSTs and rainfall in the CAB. As already mentioned a first step can be a LASSO model.
After finding a baseline model we would like to investigate how perfomance can be improved by adding more complexity, either by using more complex models and/or
using longer time ranges of data. This of course has to take into account that older data may be less reliable.  
Depending on how well this relationship can be established, we can take this 
a step further and use it as a so called "Emergent Constraint" (EC).
