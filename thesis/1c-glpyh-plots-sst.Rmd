...
seasonal_plot <- readRDS(paste0(ptp, "sst_seasonal_plot_world.rds"))
seasonal_plot + ggtitle("Glyph map of seasonal SST patterns (averages for each month)") +
...
smoothed_plot <- readRDS(paste0(ptp, "sst_smoothed_plot_world.rds"))
smoothed_plot + ggtitle("Glyph map of smoothed de-seasonalised monthly SST") +
  theme(plot.title = element_textbox_simple())
```
...
smoothed_scaled_colour_plot <- readRDS(paste0(ptp, "sst_smoothed_scaled_colour_plot_world.rds"))
...
```{r}
(sst_seasonality_window_plot <- readRDS(paste0(ptp,"sst_seasonal_plot_window.rds")))
```


```{r}
(sst_smoothed_plot_window <- readRDS(paste0(ptp, "sst_smoothed_plot_window.rds")))
```

```{r}
(sst_smoothed_scaled_colour_plot_window <- readRDS(paste0(ptp, "sst_smoothed_scaled_colour_plot_window.rds")))
```
