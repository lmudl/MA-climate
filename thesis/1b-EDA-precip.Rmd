...
The values range from 0 up to `max(precip_dens_plot$data$value)`, but only few observations take these high values, forming a large tail.
...
p <- precip_means_loc_plot +  precip_sds_loc_plot  +
  plot_annotation("Mean and SD of each location in the CAB for the complete time series")
p
```
