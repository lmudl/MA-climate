...
in both of them @tibshirani2005sparsity.
In our case the fused lasso thus penalizes the differences of SST coefficients
...
@arnold2016efficient argue that the strength of the original algorithm 
@tibshirani2011solution lays in the fact that it applies to a unified framework
...
for dual solution (note the element notation in \@ref(eq:dual).
...
Its also easy to see that in our case $p > n$ and $X$ is column rank deficient.
...
in the penalty matrix special implementations are constructed as well for our case
...
hence $m > p$. 
It can be shown that for sparse undetermined systems it is possible
to find an arbitrary solution (here called the *basic* solution), but computing the solution with minimum $l_2$
norm is a lot more difficult in general.
...
For further details of the steps used in our case refer to Section 4 and in @arnold2016efficient.
