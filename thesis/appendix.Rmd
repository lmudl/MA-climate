...
## Lasso 
### Validation scheme
```{r validation-scheme, out.width='50%'}
knitr::include_graphics("validation-scheme.png")
```

## Fused lasso
### Fused lasso, sub-graphs removed, no regularization
```{r}
path_to_model_folder <- "../results/CV-fused/noclust-large-fused-5k/"
```

```{r coef-plot-full-fused-noclust-drop-out-1, fig.cap='Coefficient plot of the full fused lasso model.Only the coefficients outside the interquantile interval are shown (quantiles of 2.5\\% and 97.5\\%).', out.width='50%'}
knitr::include_graphics(paste0(path_to_model_folder, "coef-plots/coef-plot-drop-out-full1.png"))
```

```{r coef-plot-full-fused-noclust-drop-out-2, fig.cap='Coefficient plot of the full fused lasso model.Only the coefficients inside the interquantile interval are shown (quantiles of 2.5\\% and 97.5\\%). ',  out.width='50%'}
knitr::include_graphics(paste0(path_to_model_folder, "coef-plots/coef-plot-drop-out-full2.png"))
```

### Fused lasso, sub-graphs removed, with regularization
...
```{r err-line-fused-noclust-gamma-01, out.width='45%'}
...
```{r err-line-fused-noclust-gamma-005, out.width='45%'}
knitr::include_graphics(paste0(path_to_model_folder, "best-lambda-res.png"))
```

### Fused lasso, sub-graphs included
