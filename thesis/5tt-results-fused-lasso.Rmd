...
```{r graph-plot, fig.cap='Graph of the SST and land areas used in fused lasso, colored vertices indicate sub-graphs'}
knitr::include_graphics("graph_plot.png")
```
...
for the other models to the appendix.
...
mse_full
```
On first sight the predictions don't change a lot compared to the lasso model (\@ref(fig:pred-plot-full-lasso-og)),
but the fused lasso here can predict the evaluation data best so far. (MSE is `r mse_full`,
see \@ref(fig:pred-plot-full-fused-og)). 
...
The resulting coefficient plot is given in Figure \@ref(fig:coef-plot-full-fused-og).

The coefficent plots for the fused lasso show now the regions that the fused lasso
selected (Figure \@ref:coef-plot-full-fused-noclust). 
...
In the appendix we therefore give two alternatives of this plot (Figures \@ref(fig:coef-plot-full-fused-noclust-drop-out-1) and \@ref(fig:coef-plot-full-fused-noclust-drop-out-2))
For both we computed the $2.5%$ and $97.5%$ quantiles of the coefficient values
and displayed either only the coefficients inside or outside the interquantile
interval.
...
When comparing to the coefficient plot from the lasso \@ref(fig:coef-plot-full-lasso-og),
...
```
```{r coef-plot-full-fused-noclust}
knitr::include_graphics(paste0(path_to_model_folder, "coef-plots/coef-plot-full.png"))
```
```{r coef-plot-full-fused-noclust-drop-out-1, eval=FALSE}
...
```{r coef-plot-full-fused-noclust-drop-out-2, eval=FALSE}
# coef_plot2 <- readRDS(paste0(path_to_model_folder,
#                      "coef-plots/coef-plot-drop-out-full.rds"))
#coef_plot <- coef_plot + theme(legend.position = "bottom", text = element_text(size = 8))
# coef_plot2$layers[[2]]$aes_params$size <- 0.75
...
and showed in Figure \@ref(fig:graph).
