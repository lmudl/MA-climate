...
in the Baltic sea.
We note that the largest negative and positive values occur in regions,
that have few edges in the graph.
...
```{r coef-plot-full-fused-noclust, eval=FALSE, fig.cap="Coefficient plot of the full fused lasso model."}
...
the the using the graph without separated clusters on the standard SST data
without regularization worked best.
Regularization did not improve the perfmance on the validation set.
...
The fused lasso chooses the area and distributes the coefficient value
...
correspond to land.
While doing so, areas that have only few direct connections to the large sea cluster, the 
connections get eventually cut off, leaving behind the clusters we saw earlier.
This gives the impression that the clusters are not connected at all to the larger SST
