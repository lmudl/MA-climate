...
$$ d(x_i,x_{i´}) = \sum_{i=1}^p(x_{ij}-x_{i´j})^2=||x_i-x_{i´}||^2 $$
...
of variance. (Multivariate Verfahren)
...
So the SVD gives us the matrix $V$ (the first $q$ columns give the solution to the minimization problem above) as well as the principal components from $UD$.
...
by assessing $W(C)$ or $log(W_c)$. Where are a low value indicates compact clusters. To compare the value we need a reference.
...
As Tibshirani et al. note and as it is also done in the used R function
