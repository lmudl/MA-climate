...
- These are the same data sets as in @ciemer2020early, over the time span 1981 until 2016 (36 years).
...
```{r cluster-map, echo = FALSE, out.width= '50%',fig.cap= "Spatial organization of the found clusters in the CAB. We applied a centered PCA on the data and used 3 principal components before applying the k-means algorithm"}
...
- The exception is the "island" of cluster 1 (orange) inside cluster 4 (blue) and on the edge on cluster 3 (green)
...
```{r cluster-map2, echo = FALSE, out.width= '50%', fig.cap= "Spatial organization of the found clusters in the CAB. We applied a centered PCA on the data and used 3 principal components before applying the k-means algorithm"}
km_pca_map_plot <- readRDS(paste0("../results/clustering/km_pca_map_plot.rds"))
km_pca_map_plot + 
  ggtitle("Precipitation clusters in the CAB,
after centered PCA, using 3 PC's and applying k-means")
...
- Changes in temperatures can be more meaningful in some SST regions than in others.
...
- The problem is solved by the dual path algorithm, as implemented in the *genlasso* package [@arnold2016efficient].
...
- Since no sparsity is used, all areas obtain non-zero coefficient values and some of regions with high coefficient values are included in the lasso as well in the fused lasso
